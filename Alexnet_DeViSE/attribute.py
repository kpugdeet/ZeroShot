...
class attribute (object):
    def __init__(self, numTrainClass=15):
        # All placeholder
        ...
        # Cost Function
        correctClass = tf.tile(tf.reduce_sum(tf.multiply(self.dotProduct, tf.one_hot(self.ySeen, numTrainClass)), axis=1, keep_dims=True), [1, numTrainClass])
        cost = 1.0 - correctClass + self.dotProduct
        cost = tf.maximum(cost, 0.0)
        cost = tf.multiply(cost, tf.one_hot(self.ySeen, numTrainClass, on_value=0.0, off_value=1.0))
