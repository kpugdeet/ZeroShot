...
if __name__ == "__main__":
    ...
    parser.add_argument('--NEW', type=int, default=0, help='Train from scratch')
    parser.add_argument('--PRE', type=int, default=3, help='1.CNN, 2.Model, 3.Classify')
    ...
    #     print('\nTrain classify')
    #     classifier = classify()
    #     classifier.trainClassify(allClassAtt, np.arange(allClassAtt.shape[0]))

    # # Check output
    # print('\nCheck predict output')
    # predictLabel = model.predict(tmpX[divP:divP+20], allClassAtt)
    # print('Save image to '+globalV.FLAGS.KEY+'.png')
    # plt.figure(figsize=(6, 24))
    # for i in range(20):
    #     plt.subplot(10, 2, i+1)
    #     plt.title('P: '+printClassName(predictLabel[i])+'\nA: '+printClassName(tmpY[divP+i]))
    #     plt.imshow(tmpX[divP+i], aspect='auto')
    #     plt.axis('off')
    # plt.tight_layout()
    ...
    trace = go.Heatmap(z = allClassAtt,
                       y = allClassName,
                       x = allClassAttName)
    data = [trace]
