...
class darknetModel (object):
    def __init__(self):
        ...
        if globalV.FLAGS.TD == 1:
            if tf.gfile.Exists(globalV.FLAGS.BASEDIR + globalV.FLAGS.DIR + '/darknet/logs'):
                tf.gfile.DeleteRecursively(globalV.FLAGS.BASEDIR + globalV.FLAGS.DIR + '/darknet/logs')
            tf.gfile.MakeDirs(globalV.FLAGS.BASEDIR + globalV.FLAGS.DIR + '/darknet/logs')
        self.trainWriter = tf.summary.FileWriter(globalV.FLAGS.BASEDIR + globalV.FLAGS.DIR + '/darknet/logs/train', self.sess.graph)
        self.testWriter = tf.summary.FileWriter(globalV.FLAGS.BASEDIR + globalV.FLAGS.DIR + '/darknet/logs/test')
    ...
    def restoreModel(self):
        self.saver.restore(self.sess, globalV.FLAGS.BASEDIR + globalV.FLAGS.DIR + '/darknet/model/model.ckpt')
        npzFile = np.load(globalV.FLAGS.BASEDIR + globalV.FLAGS.DIR + '/darknet/model/checkpoint.npz')
    ...
    def trainDarknet(self, trainX, trainY, valX, valY):
        ...
        for i in range(self.Start, globalV.FLAGS.maxSteps+1):
            ...
            if i % self.Check == 0:
                savePath = self.saver.save(self.sess, globalV.FLAGS.BASEDIR + globalV.FLAGS.DIR + '/darknet/model/model.ckpt', global_step=i)
                print('Model saved in file: {0}'.format(savePath))
                if i / self.Check == 10:
                    self.Check *= 10

            # Save State
            np.savez(globalV.FLAGS.BASEDIR + globalV.FLAGS.DIR + '/darknet/model/checkpoint.npz', Start=i+1, Check=self.Check)
            self.saver.save(self.sess,globalV.FLAGS.BASEDIR + globalV.FLAGS.DIR + '/darknet/model/model.ckpt')
