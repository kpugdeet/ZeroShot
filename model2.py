...
class model2 (object):
    def __init__(self):
        ...
        self.outAtt = tf.add(tf.matmul(hiddenF, wH_A), bH_A)
        ...
        self.lossAttSeen = tf.reduce_mean(tf.squared_difference(self.outAtt, self.attYSeen))
        ...
        self.trainWriter = tf.summary.FileWriter(globalV.FLAGS.BASEDIR + globalV.FLAGS.KEY + '/model2/logs/train', self.sess.graph)
        self.testWriter = tf.summary.FileWriter(globalV.FLAGS.BASEDIR + globalV.FLAGS.KEY + '/model2/logs/test')
    ...
    def restoreModel(self):
        ...
        npzFile = np.load(globalV.FLAGS.BASEDIR + globalV.FLAGS.KEY + '/model2/model/checkpoint.npz')
    ...
    def trainAtt(self, seenX, seenY, seenAttY, unseenX, unseenY, unseenAttY, allClassAtt):
        for i in range(self.Start, globalV.FLAGS.maxSteps + 1):
            ...
            for j in range(0, unseenX.shape[0], globalV.FLAGS.batchSize):
                ...
                l1, valLoss, valAccuracy = self.sess.run([self.lossAttSeen, self.lossSoftSeen, self.accuracy], feed_dict={self.x: xBatch, self.ySeen: yBatch, self.attYSeen: attYBatch,
                                                                                                                          self.allClassAtt: allClassAtt, self.isTraining: 0})
            ...
            if i % self.Check == 0:
                savePath = self.saver.save(self.sess, globalV.FLAGS.BASEDIR + globalV.FLAGS.KEY + '/model2/model/model.ckpt',global_step=i)
                print('Model saved in file: {0}'.format(savePath))
                if i / self.Check == 10:
                    self.Check *= 10

            # Save State
            np.savez(globalV.FLAGS.BASEDIR + globalV.FLAGS.KEY + '/model2/model/checkpoint.npz', Start=i+1, Check=self.Check)
            self.saver.save(self.sess, globalV.FLAGS.BASEDIR + globalV.FLAGS.KEY + '/model2/model/model.ckpt')

    def trainPrediction(self, att, y):
        for i in range(globalV.FLAGS.maxSteps):
            loss, _ = self.sess.run([self.lossSoftUnSeen, self.trainStep], feed_dict={self.attYUnseen:att, self.yUnseen: y})
            print(loss)
